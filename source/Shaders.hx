...
{
	public var shader:SolidColorShader = new SolidColorShader();

	public function new(color:FlxColor)
	{
		shader.color.value = [color.red, color.green, color.blue];
	}
}
...
class SolidColorShader extends FlxShader
{
    ...
    uniform vec3 color = vec3(0.2, 0.3, 0.2);

    void main()
    {
        vec4 form = flixel_texture2D(bitmap, openfl_TextureCoordv);
		gl_FragColor = vec4(color.r * form.a, color.g * form.a, color.b * form.a, form.a);
    }')
	public function new()
	{
		super();
